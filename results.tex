...
The failure surface calculations were performed using MATLAB\textregistered~code based on previous work by Obst \emph{et al.} \cite{Obst2018} and the mathematical relations shown in Chapter \ref{ch:oocrit}. This code can be found in Appendix \ref{ch:fsurfcode} for reference. Appendix \ref{ch:data} includes code that recreates the data from mechanical tests in a form that is easy to overlay upon the failure surface for reference.  
...
Using the results from combined loading tests plotted in the $11-12$ and $22-12$ planes allows the calculation of the slopes $\mu^{1112}$ and  $\mu^{2212}$. Beginning with the $11-12$ plane, it can be seen that applying an axial load had little to no effect on the shear strength of the samples. Using the averages for $S_{0}$ and $S_{0}^t$ yields $\mu^{1112}$ equal to $5.2\times 10^{-3}$, a value that's practically zero. Using this parameter, the failure surface shown in Figure \ref{fig:1112plane} can be obtained. A dashed line representing $\mu^{1112}$ is added for reference.
...
\begin{table} [!htbp]
	\centering
	\caption{Tensorial components obtained from tests from the $\sigma_{11}$-$\tau_{12}$ and $\sigma_{22}$-$\tau_{12}$ planes}
	\begin{tabular}{ c c } 
		\toprule
		\textbf{Component} & \textbf{Value} \\
		\midrule
...
\begin{figure}[!htbp]
	...
	\captionsetup{justification=centering} %long caption
	\caption{Complete Failure Surface} \label{fig:fullfs}
\end{figure} 
