...
The process is capable of producing complex geometries that would be otherwise hard to reproduce through other polymer processing techniques, such as injection molding. It is however bound by the disadvantages described in \ref{subsec:AMAdDis}, as well its own unique set of drawbacks. Namely:

\begin{itemize}
	\item the circular nozzle of the orifice makes FFF incapable of reproducing sharp corners, limits the size of the smallest reproducible feature, and causes the final part to be filled with voids \textendashoriginating in the junction of circular beads. These problems can be seen in Figure \ref{fig:FFFpartprob}. A 90\
\end{itemize}
\begin{figure}[h]
	\center
	...
	\caption{Typical FFF part mesostructure} \label{fig:FFFpartprob}
\end{figure}
